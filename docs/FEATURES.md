...
- **Portraits and scene artwork share one repository but not one vocabulary.**
  The labels and error-code prefix are passed in, so a portrait that fails to
  load says "a Hero portrait" and returns `portrait-read-failed`. It used to
  apologise about Scene artwork, which sent the reader to the wrong screen.
...
- **A diagonal step costs one square**, matching movement, attack range, and
  ...
  axes and answer 30 ft; a ruler that contradicts the rules engine is worse
...
- **A battle completes automatically** when one side is left standing, or when
  nobody is.
- **Every token has a side — ally or foe.** It is set in the Setup inspector,
  on the token card, and it is the fact the completion check reads. Two sides
  only: the rules need to answer "is one side left standing", and nothing in 5e
  needs a third answer. A bystander is modelled by leaving it off the board.
- **The side defaults from what the token is.** A token with a hero behind it
  starts as an ally; everything else starts as a foe. That is what every save
  written before sides existed already implied, so old battles load with the
  sides the table always understood them to have — no migration, no schema
  bump.
- **A fight where everyone shares a side falls back to last-creature-standing.**
  There are no sides to decide between, so a monster-versus-monster brawl stays
  playable instead of completing the instant initiative is rolled.
- **The winner is named** on the completion screen when a single creature is
  left, because "Wren wins" says more than "the party wins" when there is only
  one of them. When several survive, the winning side is announced instead.
...
- **Only heroes collect, and only the opposition counts.** A fallen party member
  is not treasure, an allied summon the party brought along is not treasure
  either, and a token with no hero behind it has nowhere to put a share.
...
- **Restarting resets the fight, not the world.** HP, initiative, rounds, turn
  ...
  bodies stay as they are. Spent ammunition also stays spent.
...
- **One workflow does the deploying.** `deploy-pages.yml` runs the full gate and
  picks the build base from the repository it is running in. A second workflow
  used to fire on the same push and share the same concurrency group while
  running no tests and always building the preview base — if it won the race on
  production it published unverified code whose asset paths all 404. It has
  been deleted.
